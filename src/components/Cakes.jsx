...
const Cake = () => {
  ...
  // Function to add cake to localStorage cart
  const addToCart = (cake) => {
    const cart = JSON.parse(localStorage.getItem("cart")) || [];
    const isAlreadyInCart = cart.find((item) => item.id === cake.id);
    if (!isAlreadyInCart) {
      cart.push(cake);
        ...
        {sortedCakes.map((cake) => (
          <div key={cake.id} className="cake-card">
            ...
            <div className="cake-body">
              <h6 className="cake-title" style={{ textAlign: "center" }}>{cake.name}</h6>
